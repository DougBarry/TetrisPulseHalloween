...
struct StcGamePrivate {
  ...
  uint16_t  fallingDelay; /* delay time for falling tetrominoes      */
  unsigned long lastFallTime; /* last time the falling tetromino dropped */
 ...
 */
void rotateTetromino(StcGame *game) {
  int i, j;
 ...
 */
static bool checkCollision(StcGame *game, int dx, int dy) {
  int newx, newy, i, j;
...
static void dropTetromino(StcGame *game) {
  ...
  /* Update score */
  if (game->showShadow) {
    game->stats.score += (unsigned long)(SCORE_2_FILLED_ROW * (game->stats.level + 1)
